...
CONFIG_FILE=~/SEPLOS_MQTT/config.ini
...
# Validate cell voltages of the current pack. Uses globals CELL1..CELL16
# (set in process_pack), and the pack_name + DATE for log messages.
checkcellsvoltage()
{
	STATUS=0
	counter=1
...
# Query one pack and publish its MQTT payload. Name passed as $1 maps to
# the matching <name>_DEV / _BAUD / _ADDR / _ID / _NAME block in config.ini.
process_pack() {
	local pack="$1"
...
log "$(date '+%Y-%m-%d %H:%M:%S') - Script started, packs=${PACKS}"

# Publish Home Assistant MQTT auto-discovery configs once at startup (retained).
log "$(~/SEPLOS_MQTT/publish_ha_discovery.sh)"
